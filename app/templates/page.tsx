  ...
  return (
    <div className="mx-auto max-w-6xl px-6 lg:px-8 py-12">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {TEMPLATES.map((template, i) => (
          ...
          >
            <div className="mb-4 flex items-center gap-3">
              <span
                className={`flex h-7 w-7 items-center justify-center rounded-md text-xs font-bold ${ORDER_COLORS[i] ?? "bg-muted text-muted-foreground"}`}
              >
                {i + 1}
              </span>
        ...
        <Link
          href="/templates/demo"
          className="card-hover group rounded-xl border-2 border-dashed border-border bg-muted p-6 transition hover:border-brand-300 hover:bg-brand-50/30"
        >
